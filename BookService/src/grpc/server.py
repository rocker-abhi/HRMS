...
from src.grpc.service.book_service_impl import BookServiceServicer
...
def serve():
    logger.info("Initializing Database for Book gRPC Server...")
    init_db(settings.DATABASE_URL)
    
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    book_grpc_pb2_grpc.add_BookServiceServicer_to_server(BookServiceServicer(), server)
    
    host = settings.GRPC_HOST or "localhost"
    port = settings.GRPC_PORT or 10100
    listen_addr = f"{host}:{port}"
    
    server.add_insecure_port(listen_addr)
    logger.info(f"Book gRPC Server starting on {listen_addr}...")
    server.start()
    ...
    try:
        ...
        logger.info("Book gRPC Server stopping...")
        server.stop(0)
